...
class Discriminator(nn.Module):
    ...
    def split_selfamp_obs(self, selfamp_obs, device) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        return left_obs.detach(), right_obs.detach()
    ...
    def self_amp_loss(self, left_obs, right_obs):
        ...
        return amp_loss, left_d_loss, right_d_loss, left2left.mean(dim=0), right2left.mean(dim=0), right2right.mean(dim=0), left2right.mean(dim=0)
